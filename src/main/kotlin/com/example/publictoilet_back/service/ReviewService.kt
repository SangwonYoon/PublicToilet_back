...
class ReviewService(val reviewRepository: ReviewRepository, val toiletRepository: ToiletRepository, val statisticsRepository: StatisticsRepository) {
    ...
    @Transactional
    fun save(toilet_id : Long, reviewRequestDto : ReviewRequestDto) : Long?{
        val cnt = reviewRepository.findByToiletId(toilet_id).orElseThrow {
            IllegalArgumentException("해당 화장실이 존재하지 않습니다. id=${toilet_id}")
        }.size // 해당 화장실의 리뷰 개수
        val statistics = findOrCreateStatistics(toiletRepository.findById(toilet_id).get()) // 해당 화장실의 통계 객체
        ...
        val toilet = toiletRepository.findById(toilet_id).orElseThrow {
            IllegalArgumentException("해당 화장실이 존재하지 않습니다. id=${toilet_id}")
        }
